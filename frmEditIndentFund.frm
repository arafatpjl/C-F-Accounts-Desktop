...
On Error GoTo x
    ...
    MainConn.BeginTrans
        ...
        strRecord = "Insert Into Indent_Fund_Main_bak (CompID, RefID, FundReqFor, JobNo, InvoiceNo, InvoiceValue, LCType, LCNo, BuyerID, accID, InvoiceStatus, Deleterow, Username,Userpwd, Comname, Entrydate, Entrytime) " + _
            " Values(" & bytCompID & ", " & NewID & ", '" & txtFundReqFor.Text & "', '" & txtJobNo.Text & "', '" & txtInvoiceNo.Text & "', " & CDbl(txtInvoiceValue.Text) & "," + _
            " '" & frameLCType.Tag & "', '" & txtLCNo.Text & "', " & cmbBuyerName.Tag & ", " & cmbAccHead.Tag & ", '" & frameInvoice.Tag & "',1,'" & struser & "','" & strpass & "','" & getComName & "','" & Date & "','" & Time & "' );"
        MainComm.CommandText = strRecord
        MainComm.Execute
...
'                .Col = 1:  Expensename = .text
                ...
                strRecord = strRecord + " Values (" & NewID & ", " & ExpenseID & ", " & IndentAmt & ", " & IndentAmt & ", '" & Remarks & "', " & inti & ",1,'" & struser & "','" & strpass & "','" & getComName & "','" & Date & "','" & Time & "');"
                MainComm.CommandText = strRecord
                MainComm.Execute
            Next inti
        End With
    ...
    MsgBox "Update successfully..", vbInformation, "Confirmation"
Exit Sub
x:
...
'        MainConn.BeginTrans
        ...
        R.Open strRecord, MainConn, adOpenStatic, adLockOptimistic
