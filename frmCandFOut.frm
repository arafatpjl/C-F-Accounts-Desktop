...
Begin VB.Form frmCandFOut 
   ...
   Begin VB.TextBox TxtOldVoucher 
      Alignment       =   2  'Center
      Appearance      =   0  'Flat
      BackColor       =   &H00F5F5F5&
      BeginProperty Font 
...
'         Set Report = Appl.OpenReport(ReportPath + "\statement_of_c_and_f_bills_ForwardingDate.rpt")
         ...
         Mysql = "Select * from Query_C_And_F_Out Where M_ID = " & flexDetails.Tag & " Order By SLNO;"
        
'         param = "Forwarding From Date : '" & Format(cmbForwardFromDate.Text, "DD-MMM-YYYY") & "' To Date : '" & Format(txtForwardDate.Text, "DD-MMM-YYYY") & "'"
        
        frmMainReport.CRVIEWER.ReportSource = Report
'        Report.FormulaFields(1).Text = "" & param & ""
'       Report.FormulaFields.GetItemByName("Param").Text = "'" & param & "'"
...
Private Sub cmdDelete_Click()
    ...
    MainConn.BeginTrans
        ...
        & " Select M_ID, PartyID, ForwardDate, VoucherNo, Compid, 2, '" & struser & "','" & strpass & "', '" & getComName & "', '" & Date & "', '" & Time & "' from C_And_F_Out_Main Where M_ID = " & intID & ";"
        MainComm.CommandText = Mysql
        MainComm.Execute
    
'        Mysql = "Delete from C_And_F_Out_Main Where PartyID = " & cmbPartyName.Tag & " and CompID = " & bytCompID & " and ForwardDate = '" & Trim(txtForwardDate.Text) & "';"
...
'        cmdPreview.Enabled = False
        ...
        strRecord = "SELECT DISTINCT VoucherNo FROM Query_C_And_F_Out WHERE CompID = " & bytCompID & " and PartyID = " & cmbPartyName.Tag & " And  ForwardDate = '" & Format(txtForwardDate.Text, cnstDtFrmtI) & "' ;"
...
'Dim BillNoOld2 As String
'BillNoOld2 = TxtOldVoucher.Text
''-------------------------------------
...
''------------------- INSERT STATEMENT -----------------------
    If cmdSave.Caption = "&Save" Then
        ...
        MainConn.BeginTrans
            ...
            Mysql = " Insert Into C_And_F_Out_Main_bak(M_ID, PartyID, ForwardDate, VoucherNo, Compid,Deleterow,Username,Userpwd,Comname,Entrydate,Entrytime) "
            Mysql = Mysql + " Values (" & intID & ", " & cmbPartyName.Tag & ", '" & Format(txtForwardDate.Text, cnstDtFrmtI) & "', '" & Trim(txtVoucherNo.Text) & "', " & bytCompID & ",0,'" & struser & "','" & strpass & "','" & getComName & "','" & Date & "','" & Time & "');"
            MainComm.CommandText = Mysql
            MainComm.Execute
            ...
            With flexDetails
                For i = 1 To .Rows - 1
                    ...
                    Mysql = " Insert Into C_And_F_Out_Sub_bak (M_ID, BillNO, BillDate, HAWB, BENO, BEDate, BillAmount, ApprovedAmount, AIT, VAT, TotalAmount, SLNO,Deleterow,Username,Userpwd,Comname,Entrydate,Entrytime)"
                    Mysql = Mysql + " Values ( " & intID & ", '" & BillNo & "', '" & BillDate & "', '" & HAWB & "', '" & BENO & "', '" & BEDate & "', " & BillAmount & ", " & AppAmount & ", " & AIT & ", " & VAT & ", " & Total & ", " & i & ",0,'" & struser & "','" & strpass & "','" & getComName & "','" & Date & "','" & Time & "');"
                    MainComm.CommandText = Mysql
                    MainComm.Execute
                Next
            End With
...
''------------------- UPDATE STATEMENT -----------------------
        ...
        MainConn.BeginTrans
            ...
            Mysql = " Insert Into C_And_F_Out_Main_bak(M_ID, PartyID, ForwardDate, VoucherNo, Compid,Deleterow,Username,Userpwd,Comname,Entrydate,Entrytime) "
            Mysql = Mysql + " SELECT M_ID, PartyID, ForwardDate, VoucherNo, Compid,1,'" & struser & "','" & strpass & "','" & getComName & "','" & Date & "','" & Time & "' FROM C_And_F_Out_Main Where M_ID = " & intID & ";"
            MainComm.CommandText = Mysql
            MainComm.Execute
            ...
            With flexDetails
                For i = 1 To .Rows - 1
                    ...
                    Mysql = " Insert Into C_And_F_Out_Sub_bak (M_ID, BillNO, BillDate, HAWB, BENO, BEDate, BillAmount, ApprovedAmount, AIT, VAT, TotalAmount, SLNO,Deleterow,Username,Userpwd,Comname,Entrydate,Entrytime)"
                    Mysql = Mysql + " Values ( " & intID & ", '" & BillNo & "', '" & BillDate & "', '" & HAWB & "', '" & BENO & "', '" & BEDate & "', " & BillAmount & ", " & AppAmount & ", " & AIT & ", " & VAT & ", " & Total & ", " & i & ",1,'" & struser & "','" & strpass & "','" & getComName & "','" & Date & "','" & Time & "');"
                    MainComm.CommandText = Mysql
                    MainComm.Execute
                Next
            End With
...
Private Sub flexDetails_KeyDown(KeyCode As Integer, Shift As Integer)
    If KeyCode = 13 Then
        With flexDetails
           ...
           'flexDetails.TopRow = flexDetails.Row
...
fncDuplicate = True
    Set tmprst = New Recordset
        Mysql = "SELECT Count(*) AS RC FROM C_And_F_Out_Main WHERE PartyID = " & cmbPartyName.Tag & " and Compid = " & bytCompID & " And VoucherNo = '" & txtVoucherNo.Text & "'"
        'MainComm.CommandText = Mysql
...
'        cmdPreview.Enabled = False
        ...
        strRecord = "SELECT M_ID, PartyID, PartyName, ForwardDate, VoucherNo, BillNo, BillDate, HAWB, BENO, BEDate, BillAmount, ApprovedAmount, AIT, VAT, TotalAmount, SLNO FROM Query_C_And_F_Out WHERE CompID = " & bytCompID & " and PartyID = " & cmbPartyName.Tag & " And  ForwardDate = '" & Format(txtForwardDate.Text, cnstDtFrmtI) & "' AND VoucherNo = '" & txtVoucherNo.Text & "' order by SLNO;"
        'MainComm.CommandText = Mysql
        'Set tmprst = MainComm.Execute
        tmprst.Open strRecord, MainConn, adOpenStatic
...
Private Sub txtVoucherNo_Click()
Call prcLoadData
TxtOldVoucher.Text = txtVoucherNo.Text
End Sub
...
Private Sub txtBEno_LostFocus()
 ...
 strRecord = "SELECT BENO FROM C_And_F_Out_Sub INNER JOIN C_And_F_Out_Main ON C_And_F_Out_Main.M_ID=C_And_F_Out_Sub.M_ID WHERE C_And_F_Out_Sub.BENO='" & txtBENO.Text & "' AND C_And_F_Out_Main.PartyID=" & cmbPartyName.Tag & ";"
            R.Open strRecord, MainConn, adOpenStatic
            If R.RecordCount > 0 Then
                MsgBox "Duplicate BENO", vbInformation, cnstMsgInfo
                txtBENO.SetFocus
            Exit Sub
            End If
            R.Close
...
Public Function fncFindID() As Integer
    Set tmprst = New Recordset
    Mysql = "SELECT M_ID  FROM Query_C_And_F_Out Where PartyID = " & cmbPartyName.Tag & " and Compid = " & bytCompID & " and ForwardDate = '" & Trim(txtForwardDate.Text) & "' AND VoucherNO = '" & TxtOldVoucher.Text & "' ;"
...
Private Sub prcTotal()
    ...
    txtTotal.Text = FormatNumber(Round(AA - (AIT + VAT), 2), 0)
'    txtTotal.Text = FormatNumber(AA - (AIT + VAT), 2)
End Sub
