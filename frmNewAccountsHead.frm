VERSION 5.00
Begin VB.Form frmNewAccountsHead 
   BackColor       =   &H00EAF5F5&
   ...
   ClientTop       =   2190
   ClientWidth     =   3855
   Icon            =   "frmNewAccountsHead.frx":0000
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   2760
   ScaleWidth      =   3855
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.Label Label1 
         ...
         Caption         =   "Nature Of The Accounts Head :"
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   8.25
            Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmNewAccountsHead"
...
On Error GoTo x
    ...
    If Len(txtDist) = 0 Then
        MsgBox "Enter Nature Of Expense.", vbInformation, AppTitle
        txtDist.SetFocus
    Exit Sub
    End If
        ...
        MainConn.BeginTrans
            If flagMark = "INSERT" Then
                NewID = fncNewID("ACCHEAD")
                MainComm.CommandText = "INSERT INTO New_ACC_HEAD( ACCID, ACCName) VALUES(" & NewID & ", '" & txtDist.Text & "');"
                MainComm.Execute
            Else
                MainComm.CommandText = "UPDATE New_ACC_HEAD SET ACCName='" & txtDist.Text & "' WHERE ACCID=" & Tag & ";"
                MainComm.Execute
            End If
...
Private Sub Form_Unload(Cancel As Integer)
    Set frmNewAccountsHead = Nothing
End Sub
...
fncDuplicate = True
    ...
    If flagMark = "INSERT" Or (Trim(txtDist.Text) = Trim(txtDist.Tag)) Then
        strRecord = "SELECT Count(*) FROM new_ACC_HEAD WHERE (ACCName='" & txtDist.Text & "');"
        MainComm.CommandText = strRecord
        Set R = MainComm.Execute
        If R.Fields(0) > 0 Then
            MsgBox "Name of the accounts head  '" & txtDist.Text & "' already exist." & vbCrLf & "Change the name of the accounts head.", vbInformation, AppTitle
            txtDist.SetFocus
        Exit Function
        End If
    End If
