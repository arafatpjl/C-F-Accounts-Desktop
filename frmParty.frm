...
On Error GoTo x
    If MsgBox("Are you sure to delete" & vbCrLf & " Buyer '" & flexDetails.Text & "' ?", vbQuestion + vbYesNo, AppTitle) = vbNo Then Exit Sub
        MainConn.BeginTrans
            MainComm.CommandText = "DELETE FROM new_Party WHERE PartyName='" & flexDetails.Text & "';"
            MainComm.Execute
...
On Error GoTo x
    ...
    Dim inti As Integer
    strRecord = "SELECT PartyID, PartyName FROM new_Party ORDER BY PartyName;"
