...
Begin VB.Form frmAdjustmentDetails 
   ...
   Begin VB.TextBox txtOtherAdjAmount 
      ...
      TabIndex        =   11
      Text            =   "0.00"
      ToolTipText     =   "Type Correct Cash Adjust Value"
      Top             =   330
      Width           =   1545
   End
   ...
   Begin VB.TextBox txtAdjDate 
      ...
      Left            =   1770
      TabIndex        =   0
      ToolTipText     =   "Type Adjustment Date"
      Top             =   330
      Width           =   1545
   End
...
If MsgBox("Are you sure to Update", vbQuestion + vbYesNo + vbDefaultButton1, cnstMsgQ) = vbNo Then Exit Sub
    ...
    If frmEditIndentEntry.optImport.Value = True Then
        MainComm.CommandText = "UPDATE new_Indent_Information SET AdjDate='" & Format(txtAdjDate.Text, cnstDtFrmtI) & "',AdjAmount=" & CCur(txtAdjAmount.Text) & ",BAdjAmount=" & CCur(txtBAdjAmount.Text) & ",OtherAdjAmount=" & CCur(txtOtherAdjAmount.Text) & ",TotAdjust=" & CCur(txtAdjAmount.Text) + CCur(txtBAdjAmount.Text) & ",TotBAmount=" & StrTotalIndent & " WHERE ((compID=" & bytCompID & ") AND (JobNo='" & txtJobNo.Text & "') AND (Status='I')) ;"
        MainComm.Execute
        MainComm.CommandText = "Insert into new_Indent_Information_bak(compID, JobNo, InvNo, PayDate,OtherPayDate, TotIndValue, BPayDate, BPayValue, IndAmount, AccID, Remarks, Status, AdjDate, TotBAmount, AdjAmount,BAdjAmount,OtherAdjAmount, TotAdjust, AdjTax, AdjVat, Deleterow, Username, Userpwd, Comname, Entrydate, Entrytime)" _
                      & " SELECT compID, JobNo, InvNo, PayDate,OtherPayDate, TotIndValue, BPayDate, BPayValue, IndAmount, AccID, Remarks, Status, AdjDate, TotBAmount, AdjAmount,BAdjAmount,OtherAdjAmount, TotAdjust, AdjTax, AdjVat, 1, '" & struser & "', '" & strpass & "', '" & getComName & "', '" & Date & "','" & Time & "' FROM new_Indent_Information WHERE ((compID=" & bytCompID & ") AND (JobNo='" & txtJobNo.Text & "') AND (Status='I')) ;"
        MainComm.Execute
    Else
        MainComm.CommandText = "UPDATE new_Indent_Information SET AdjDate='" & Format(txtAdjDate.Text, cnstDtFrmtI) & "',AdjAmount=" & CCur(txtAdjAmount.Text) & ",BAdjAmount=" & CCur(txtBAdjAmount.Text) & ",OtherAdjAmount=" & CCur(txtOtherAdjAmount.Text) & ",TotAdjust=" & CCur(txtAdjAmount.Text) + CCur(txtBAdjAmount.Text) & ",TotBAmount=" & StrTotalIndent & " WHERE ((compID=" & bytCompID & ") AND (JobNo='" & txtJobNo.Text & "') AND (Status='E')) ;"
        MainComm.Execute
        MainComm.CommandText = "Insert into new_Indent_Information_bak(compID, JobNo, InvNo, PayDate, TotIndValue, BPayDate,OtherPayDate, BPayValue, IndAmount, AccID, Remarks, Status, AdjDate, TotBAmount, AdjAmount,BAdjAmount,OtherAdjAmount, TotAdjust, AdjTax, AdjVat, Deleterow, Username, Userpwd, Comname, Entrydate, Entrytime)" _
                      & " SELECT compID, JobNo, InvNo, PayDate, TotIndValue, BPayDate,OtherPayDate, BPayValue, IndAmount, AccID, Remarks, Status, AdjDate, TotBAmount, AdjAmount,BAdjAmount,OtherAdjAmount, TotAdjust, AdjTax, AdjVat, 1, '" & struser & "', '" & strpass & "', '" & getComName & "', '" & Date & "','" & Time & "' FROM new_Indent_Information WHERE ((compID=" & bytCompID & ") AND (JobNo='" & txtJobNo.Text & "') AND (Status='E')) ;"
    MainComm.Execute
    End If
...
ErrorHandler:
    ...
    frmEditIndentEntry.OtherAdjDate.Text = "": frmEditIndentEntry.OtherTotalAdjAmount.Text = "0.00"
    Screen.MousePointer = vbDefault
End Sub
